...
class ValidationVisualizerV2MVDRHook(BaseHook):
    """
    ...
    def after_val_iter(self, runner):
        ...
        for i in range(input_tensor.size(0)):
            ...
            # --- 准备三帧原图 V2+ATTENTION特殊处理---
            # 输入形状为 [b, 9, h, w]，因为三帧RGB图像 (3帧 * 3通道)
            frame_indices = [
                (0, 3), # Frame 1: 通道 0, 1, 2
                (5, 8), # Frame 2: 通道 5, 6, 7
                (10, 13) # Frame 3: 通道 10, 11, 12
            ]

            input_frames = []
            for start_idx, end_idx in frame_indices:
                # 提取当前批次、特定通道范围的张量
                # input_tensor[i, start_idx:end_idx, :, :] 形状为 [3, H, W]
                
                # 1. 提取 (C, H, W)  
                frame_rgb_tensor = input_tensor[i, start_idx:end_idx, :, :]
                
                # 2. 转换为 (H, W, C) numpy
                frame_rgb = frame_rgb_tensor.permute(1, 2, 0).numpy()
                
                # 3. 反归一化并转换为 uint8
                frame_rgb = (frame_rgb * 255).astype(np.uint8)
                
                # 4. 转换为 BGR 格式 (cv2 默认使用 BGR)
